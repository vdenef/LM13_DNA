...
pdf(file="Figures/Rplot_BetaDiv_boxplot_combined_2.pdf", width= 7, height=3.5, pointsize= 8, colormodel="cmyk", useDingbats = FALSE)
grid.arrange(bray_betweenstat_boxplot,bray_wifrac_boxplot,ncol=2)
dev.off()
...
facet_grid(Depth_C ~.) +
ylab("rel. abund") + #scale_color_manual(values = station_colors) +
...
#prep dataframe
select <-physeq_dupmerge.prune %>%
  transform_sample_counts(function(x) {x/sum(x)}) %>%
  subset_taxa(Tribe == "LD12"|Tribe == "Lhab-A1"|Genus == "Nitrosospira"|Phylum =="Chloroflexi") %>%
  psmelt()
...
select_plots <-lapply(select_names,
  function(otu) {
    ...
    plot <- plot_otus(df = df_otu, otu = otu, taxrank = "Tribe")
    return(plot)
  }
)

pdf(file="Figures/Rplot_lineplots.pdf", width= 7, height=7, pointsize= 8, colormodel="cmyk", useDingbats = FALSE)
grid.arrange(select_plots$Otu000002, select_plots$Otu105919, select_plots$Otu000119,select_plots$Otu000021,ncol = 2)
dev.off()
```
...
```{r final figure PA vs FL betadiv + DEseq}
library(grid)
library(gridExtra)
