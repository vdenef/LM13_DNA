...
sigtab.LMvsMLB <- cbind(sigtab, 
  as(tax_table(physeq_dupmerge.prune)[sigtab$OTU, ], "matrix"),fungroupfile[sigtab$OTU, ])
...
plotvalues <- data.frame(data.frame(tax_table(physeq_dupmerge.prune))$Species,as.vector(as.numeric(taxa_sums(physeq_dupmerge.prune)/sum(sample_sums(physeq_dupmerge.prune))*100)^.25))
colnames(plotvalue) <- c("Species","abundance")
row.names(plotvalue) <- plotvalue$Species
sigtab.LMvsMLB <- cbind(sigtab, 
  as(tax_table(physeq_dupmerge.prune)[sigtab$OTU, ], "matrix"),fungroupfile[sigtab$OTU, ],plotvalue[sigtab$OTU, ])
...
plot.LMvsMLB <- ggplot(sigtab.LMvsMLB, 
  ...
  geom_jitter(position = position_jitter(height = 0.25),alpha=0.7) + 
...
plot.SvsD_wlegend <- ggplot(sigtab.SvsD, 
  ...
  scale_shape_manual(name = "p-value", breaks = c("0", "1"), 
                    labels = c("p>0.01", "p<0.01"),
                    values = c("0" = 21, "1"= 19)) +
  xlab("log2fold Surface:Deep") + ylab("Functional group") +
  theme(plot.title = element_text(face="bold", size=7),  #Set the plot title
                                 strip.text.x = element_blank(),  #Set the facet titles on x-axis 
                                 strip.text.y = element_blank(),  #Set the facet titles on x-axis 
                                 strip.background = element_blank(),  #Set the facet background to no background
...
grid.arrange(plot.LMvsMLB,plot.FLvsPA,plot.SuvsSp,plot.SuvsFa,plot.SvsD,plot.legend, ncol=2)
dev.off()
